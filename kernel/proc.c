...
found:
  ...
  // an empty kernel page table for every proc.
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        // write kpgtbl to stap and flush TLB
        // preference kvminithart()
